...
\begin{table}[!htbp]
\centering
\caption[Theoretical Research Matrix with Platform Applications]{Theoretical Research Matrix showing mapping of literature to Consol platform capabilities}
...
SimCSE introduces contrastive learning specifically designed for sentence embedding optimization, establishing both unsupervised and supervised variants. The unsupervised approach leverages dropout as implicit data augmentation, treating the same input sentence with different dropout masks as positive pairs. The supervised version incorporates natural language inference datasets where entailment pairs serve as positives and contradiction pairs as hard negatives.
...
For supervised SimCSE, the objective incorporates labeled pairs where $(x_i, x_i^+)$ represents entailment pairs and hard negatives $x_i^-$ from contradiction relationships:

\begin{equation}
\ell_i = -\log \frac{e^{\text{sim}(h_i, h_i^+)/\tau}}{\sum_{j=1}^{N} e^{\text{sim}(h_i, h_j^+)/\tau} + e^{\text{sim}(h_i, h_i^-)/\tau}}
\end{equation}

This supervised approach produces superior performance by explicitly teaching the model to distinguish between semantic entailment and contradiction relationships.
...
Consol implements supervised SimCSE models pre-trained on natural language inference datasets to ensure optimal semantic similarity measurement for educational assessment. The platform uses the 'all-mpnet-base-v2' model which achieves state-of-the-art performance on semantic textual similarity benchmarks through SimCSE training methodology.
...
\section{Theoretical Architecture Integration}
